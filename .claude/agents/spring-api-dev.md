...
作業前に必ず読むこと:
1. `.claude/claude.md`（アーキテクチャ・ErrorCode・状態遷移・進捗）
2. `claude_code_handoff_daipetto.md`（特に §7〜§19 と §22）
3. 実際のリポジトリコード（文書とコードが食い違う場合はコードを確認してから判断する）
...
- DB 変更は Flyway migration のみ。**採番前に既存ブランチとの番号衝突を確認**（V4 は未マージの feat/spring/pet-fields が使用済み。V7 以降を使う）
...
- 現在の練習用未実装（**頼まれない限り実装しないこと**）: 病院更新 / 病院停止 / 予約枠登録 / 予約枠 AVAILABLE-BLOCKED 切替
