---
name: docs-sync
description: 実装変更後の docs/ 全13文書・TODO.md・ルート CLAUDE.md §10 の同期更新を担当。API/ERD/状態遷移/ErrorCode/環境バージョンの変更を文書へ反映する作業、文書と実装の整合性監査で使用する。
---
...
ルート `CLAUDE.md` のみ英語（Claude Code 用コンテキストのため）。
...
1. `TODO.md`（完了チェック・「あえて未実装のまま残した項目（練習用）」の維持）
2. 該当する `docs/` 文書
3. ルート `CLAUDE.md`（§5 ErrorCode / §7 Flyway / §10 進捗 など該当セクション）
...
残存する既知ギャップはルート `CLAUDE.md` §12 の "Known remaining gaps" が正。
（2026-08-23 の文書全数監査で docs 02〜13 の既知の不整合は修正済み。）
