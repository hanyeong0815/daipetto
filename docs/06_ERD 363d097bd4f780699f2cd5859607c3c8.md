...
| **論理テーブル名** | **物理テーブル名** | **説明** | **Migration** |
| --- | --- | --- | --- |
| ユーザー | users | システム利用者情報 | V1 |
| リフレッシュトークン | refresh_tokens | Refresh Token管理 | V2 |
| ペット | pets | ユーザーが飼育するペット情報 | V3 |
| 病院 | hospitals | 動物病院情報 | V4 |
| 病院営業時間 | hospital_business_hours | 病院の曜日別デフォルト営業時間（予約枠自動生成の元データ） | V5 |
| 病院予約枠 | hospital_schedules | 病院の予約可能時間 | V6 |
| 予約 | reservations | 病院予約情報 | V7 |
| 病院管理者 | hospital_admins | 病院と管理者の紐付け | 未作成 |
| 健康記録 | health_records | ペットの健康記録 | 未作成 |
| ワクチン接種予定 | vaccinations | ワクチン接種予定・完了情報 | 未作成 |
| 通知 | notifications | ユーザー向け通知情報 | 未作成 |
...
> **注記:** `breed` / `neutered` / `microchip_number` カラムは未マージの `feat/spring/pet-fields` ブランチ（`V4__alter_pets_add_columns.sql`）で追加予定。マージ時に migration 番号の衝突（現ブランチの V4 は hospitals が使用）を解消してから本表へ反映すること。
